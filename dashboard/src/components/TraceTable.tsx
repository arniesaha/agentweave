...
function AttributeGrid({ attrs }: { attrs: Record<string, string> }) {
  ...
  if (!entries.length) return <p className="text-ink-faint text-xs italic">No attributes</p>
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
      {entries.map(([k, v]) => (
        <div key={k} className="bg-void rounded-lg p-2">
          <div className="text-ink-faint text-xs font-medium truncate">{k}</div>
          <div className="text-ink text-xs mt-0.5 break-all">{v}</div>
        </div>
      ))}
    </div>
  )
...
export function TraceTable({ traces, loading, error }: TraceTableProps) {
  ...
  const ThSortable = ({ label, k }: { label: string; k: SortKey }) => (
    <th
      className="px-4 py-3 text-left text-[10px] font-medium text-ink-faint uppercase tracking-wider cursor-pointer hover:text-ink-muted select-none whitespace-nowrap"
  ...
  return (
    <div className="card overflow-hidden">
      <div className="px-5 py-4 border-b border-edge flex items-center justify-between">
        <div>
          <h3 className="text-ink font-semibold text-sm">Recent LLM Calls</h3>
          <p className="text-ink-muted text-xs mt-0.5">Latest traces from Tempo — click row to expand</p>
        </div>
        {!loading && !error && (
        ...
        <table className="w-full">
          <thead className="bg-void/50">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-[10px] font-medium text-ink-faint uppercase tracking-wider">Trace ID</th>
              <th className="px-4 py-3 text-left text-[10px] font-medium text-ink-faint uppercase tracking-wider">Model</th>
              <th className="px-4 py-3 text-left text-[10px] font-medium text-ink-faint uppercase tracking-wider">Agent</th>
              ...
              <th className="px-4 py-3 text-left text-[10px] font-medium text-ink-faint uppercase tracking-wider">Session</th>
              <th className="px-4 py-3 text-left text-[10px] font-medium text-ink-faint uppercase tracking-wider" title="PII detected in this span">PII</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={11} className="px-4 py-12 text-center text-ink-faint text-sm">
              ...
              pageRows.map((row) => {
                ...
                return (
                  <React.Fragment key={row.traceId}>
                    <tr
                      className="border-b border-edge hover:bg-surface-overlay/30 cursor-pointer transition-colors"
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <a
                          ...
                          className="flex items-center gap-1 text-accent hover:text-accent/80 text-xs mono"
                      ...
                      <td className="px-4 py-3">
                        <span className="text-xs text-accent bg-accent/8 px-2 py-0.5 rounded-full">
                          {row.agentId || '—'}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-xs text-ink mono whitespace-nowrap">
                        {row.latencyMs > 0 ? `${row.latencyMs.toFixed(0)} ms` : '—'}
                      </td>
                      <td className="px-4 py-3 text-xs text-ink-muted mono">
                      ...
                      <td className="px-4 py-3">
                        {row.sessionId && row.sessionId !== '—' ? (
                          <button
                            className="text-xs text-signal-sky hover:text-signal-sky/80 mono max-w-24 truncate block transition-colors underline decoration-dotted"
                      ...
                      <td className="px-4 py-3">
                        {row.piiDetected ? (
                          <span
                            ...
                            className="inline-flex items-center gap-1 text-xs text-signal-amber bg-signal-amber/8 border border-signal-amber/20 px-1.5 py-0.5 rounded-full font-medium"
                          >
        ...
        <div className="px-5 py-3 border-t border-edge flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              disabled={page === 0}
              className="p-1.5 rounded text-ink-muted hover:text-ink hover:bg-surface-overlay disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            >
